...
- **`questions.json`** (Claude Certified Architect - Foundations, CCAR-F) and **`ccar-p.json`** (Claude Certified Architect - Professional, CCAR-P) are the two question banks, each an array of question objects with the same shape (`id`, `domain`, `scenario`, optional `constraints`, `question`, `options` (`key`/`text` pairs), `correct` (the right option's key), `correctReason`, and `whyFail` (a reason per incorrect key)). Question `id`s must stay unique within a file but do not need to be unique across the two files. `scenario`/`constraints` values are injected via `innerHTML` (safe here since content is authored, not user-supplied), so they can include inline `<em>` for emphasis.
